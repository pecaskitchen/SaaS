...
async function readMenuOverrides(env) {
  try {
...
async function auth(request, env) {
  ...
  if (env.SUPER_PASSWORD && password === env.SUPER_PASSWORD) return true;
  return false;
}
...
export async function onRequestGet({ request, env }) {
  try {
    ...
    if (!(await auth(request, env))) return jsonResponse({ ok: false, error: 'No autorizado.' }, 401);
    ...
    if (type === 'sales_products') {
      const sql = `SELECT o.created_at_monterrey AS fecha, o.branch_name AS sucursal, o.order_source AS origen, oi.product_name AS producto, oi.category AS categoria, SUM(oi.quantity) AS cantidad, SUM(oi.line_total) AS venta_total FROM order_items oi JOIN orders o ON o.id = oi.order_id WHERE o.created_at_monterrey BETWEEN ? AND ?${branch.sql.replace('branch_id','o.branch_id')} GROUP BY fecha, sucursal, origen, producto, categoria ORDER BY fecha DESC, producto ASC`;
      const result = await env.DB.prepare(sql).bind(startDateTime, endDateTime, ...branch.binds).all();
    ...
    if (type === 'source_summary') {
      const sql = `SELECT SUBSTR(created_at_monterrey, 1, 10) AS fecha, branch_name AS sucursal, order_source AS origen, COUNT(*) AS pedidos, SUM(total) AS venta_total FROM orders WHERE created_at_monterrey BETWEEN ? AND ?${branch.sql} GROUP BY fecha, sucursal, origen ORDER BY fecha DESC, sucursal ASC, origen ASC`;
      const result = await env.DB.prepare(sql).bind(startDateTime, endDateTime, ...branch.binds).all();
    ...
    if (type === 'stock_movements') {
      ...
      const sql = `SELECT m.created_at_monterrey AS fecha, m.branch_name AS sucursal, i.name AS ingrediente, m.movement_type AS tipo_movimiento, m.quantity AS cantidad, m.stock_before, m.stock_after, m.reason AS motivo, m.reported_by AS usuario, m.reported_role AS rol, m.source_type, m.source_id FROM stock_movements m JOIN inventory_items i ON i.id = m.item_id WHERE m.created_at_monterrey BETWEEN ? AND ?${stockBranch.sql} ORDER BY m.created_at_monterrey DESC`;
      const result = await env.DB.prepare(sql).bind(startDateTime, endDateTime, ...stockBranch.binds).all();
    ...
    if (type === 'waste') {
      ...
      const sql = `SELECT w.created_at_monterrey AS fecha, w.branch_name AS sucursal, i.name AS ingrediente, w.quantity AS cantidad, w.reason AS motivo, w.status AS estado, w.reported_by AS reportado_por, w.approved_by AS aprobado_por FROM waste_requests w JOIN inventory_items i ON i.id = w.item_id WHERE w.created_at_monterrey BETWEEN ? AND ?${wasteBranch.sql} ORDER BY w.created_at_monterrey DESC`;
      const result = await env.DB.prepare(sql).bind(startDateTime, endDateTime, ...wasteBranch.binds).all();
    ...
    if (type === 'purchase_suggestions') {
      ...
      const sql = `SELECT ? AS sucursal, COALESCE(s.name, 'Sin proveedor') AS proveedor, i.name AS ingrediente, COALESCE(bs.current_stock, i.current_stock, 0) AS stock_actual, i.min_stock AS minimo, i.max_stock AS maximo, CASE WHEN COALESCE(bs.current_stock, i.current_stock, 0) < i.min_stock THEN MAX(i.max_stock - COALESCE(bs.current_stock, i.current_stock, 0), 0) ELSE 0 END AS sugerido_comprar, u.code AS unidad FROM inventory_items i JOIN stock_units u ON u.id = i.unit_id LEFT JOIN inventory_branch_stock bs ON bs.item_id = i.id AND bs.branch_id = ? LEFT JOIN stock_suppliers s ON s.id = i.primary_supplier_id WHERE i.is_active = 1 AND COALESCE(bs.current_stock, i.current_stock, 0) < i.min_stock ORDER BY proveedor ASC, ingrediente ASC`;
      const result = await env.DB.prepare(sql).bind(selectedBranch, selectedBranch).all();
    ...
    if (type === 'category_sales') {
      const sql = `SELECT SUBSTR(o.created_at_monterrey, 1, 10) AS fecha, o.branch_name AS sucursal, oi.category AS categoria, SUM(oi.quantity) AS unidades, SUM(oi.line_total) AS venta_total FROM order_items oi JOIN orders o ON o.id = oi.order_id WHERE o.created_at_monterrey BETWEEN ? AND ?${branch.sql.replace('branch_id','o.branch_id')} GROUP BY fecha, sucursal, categoria ORDER BY fecha DESC, venta_total DESC`;
      const result = await env.DB.prepare(sql).bind(startDateTime, endDateTime, ...branch.binds).all();
      return csvResponse(`pecas-ventas-categoria-${start}-${end}.csv`, ['fecha','sucursal','categoria','unidades','venta_total'], result.results || []);
    }

    if (type === 'branch_summary') {
      const sql = `SELECT branch_name AS sucursal, COUNT(*) AS pedidos, SUM(total) AS venta_total, SUM(CASE WHEN order_source = 'online' THEN 1 ELSE 0 END) AS pedidos_online, SUM(CASE WHEN order_source = 'cashier' THEN 1 ELSE 0 END) AS pedidos_caja, AVG(total) AS ticket_promedio FROM orders WHERE created_at_monterrey BETWEEN ? AND ?${branch.sql} GROUP BY branch_name ORDER BY venta_total DESC`;
      const result = await env.DB.prepare(sql).bind(startDateTime, endDateTime, ...branch.binds).all();
    ...
    if (type === 'inventory_value') {
      const selectedBranch = branchId === 'all' ? 'dominio' : branchId;
      const sql = `SELECT ? AS sucursal, i.name AS ingrediente, u.code AS unidad, COALESCE(bs.current_stock, i.current_stock, 0) AS stock_actual, i.purchase_unit_quantity AS cantidad_presentacion, i.purchase_price AS precio_presentacion, CASE WHEN COALESCE(i.purchase_unit_quantity, 0) > 0 THEN ROUND(COALESCE(bs.current_stock, i.current_stock, 0) * COALESCE(i.purchase_price, 0) / i.purchase_unit_quantity, 2) ELSE 0 END AS valor_estimado FROM inventory_items i LEFT JOIN stock_units u ON u.id = i.unit_id LEFT JOIN inventory_branch_stock bs ON bs.item_id = i.id AND bs.branch_id = ? WHERE i.is_active = 1 ORDER BY valor_estimado DESC, ingrediente ASC`;
      const result = await env.DB.prepare(sql).bind(selectedBranch, selectedBranch).all();
    ...
    if (type === 'sold_out') {
      const overrides = await readMenuOverrides(env);
